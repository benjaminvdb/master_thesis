...
The task is often referred to as \emph{Names-Entity Recognition} (NER)\todo{add reference} and usually solved with either grammars or statistical (machine learning) approaches.
...
The method that we settled on using is a simplified version of a grammar based approach\todo{add reference} and mainly requires a list of first names as a \emph{seed}.
...
Repeated application of this procedure can consequently be used to expand a relatively small list of known names into a bigger one in a process called \emph{bootstrapping}.\todo{referenen aan experiment}
...
An example of a very simple blocking key would be to take the first character of the last name or to combine the first characters of both the first and last names.
...
Blocking also indirectly affects the specificity of the system since it allows for more expensive, and thus more thorough, candidate pair comparisons, which in theory yield more accurate results \citep{Baxter2003}.
